...
### Primary Persona: "Emma" — Young Professional
...
### Secondary Persona: "Marcus" — Small Business Owner
...
### Tertiary Persona: "Sofia" — College Student
...
6. **Settings & Profile**
   ...
   - Acceptance Criteria:
     ...
     - [x] Select preferred currency (USD, EUR, GBP, INR, JPY, CAD, AUD, SGD) — persists in localStorage, propagates globally
     - [x] Select preferred date format (MM/DD/YYYY, DD/MM/YYYY, YYYY-MM-DD) — persists in localStorage, applies across all pages
...
4. **Multi-Currency Support** ✅ *Implemented*
   - Default currency: USD (`$`), locale `en-US`
   - User-selectable in Settings: USD, EUR, GBP, INR, JPY, CAD, AUD, SGD
   - Selected currency stored in localStorage (`STORAGE_KEYS.SETTINGS`)
   - Exposed globally via `FinanceContext` → all pages/components consume via `useFinance()`
   - `formatCurrency(amount, currency)` utility in `utils/helpers.js` (locale `en-US`, default `$`)
   - All charts receive `currency` prop dynamically from their parent pages
   - localStorage migration: on load, stale `₹` auto-migrated to `$`; `DD/MM/YYYY` auto-migrated to `MM/DD/YYYY`
...
| **Hero** | Value proposition, animated stats (100K+ users, $500M+ tracked, 4.9/5 rating), cinematic video bg |
| **Features** | 6 feature cards: Expense Tracking, Budget Planning, AI Insights, Reports, Bill Reminders, Financial Goals |
| **3D Showcase** | Animated dashboard mockup with video bg, tech stats, holographic scanner |
| **How It Works** | 4-step onboarding: Create Account → Connect Accounts → Track & Analyze → Achieve Goals |
| **Pricing** | 3 tiers: Free ($0), Pro ($9.99/mo), Business ($29.99/mo). 14-day trial, no refunds after payment |
| **Review** | Community Feedback section with video bg, 3 initial reviews, user review submission form |
| **FAQ** | 16 FAQs (account creation, 2FA setup, password reset, data safety, budgets, exports) |
| **CTA** | Dark amber theme, mouse-tracking glow, "Stop Wondering. Start Growing." trust badges |
| **Footer** | 4-column links, Global Community hub (12 social platforms), newsletter, world map |

**Website features NOT in scope**: The website includes a dashboard route and 16 standalone pages (Privacy, Terms, About, etc.) but no backend data processing, no actual transaction management, no live authentication. Those are mobile-app-only.

## 8. User Scenarios

### Scenario 1: Daily Expense Logging
- **Context**: Emma buys coffee and lunch during work
- **Steps**:
  1. Opens FinTrack app
  2. Taps "Add Transaction" on Dashboard or Transactions screen
  3. Selects "Expense" type
  4. Enters $15, category "Food & Dining", description "Lunch"
...
- **Context**: End of month, Marcus checks budget utilization
- **Steps**:
...
- **Context**: Sofia checks how her first stock is performing
- **Steps**:
...
Context:
- Target Users: Young professionals, small business owners, students worldwide
...
- **Direct URL**: `https://fintrack.app` → Hero section
...
2. **Action: Add Budget**
   ...
   - User: Selects "Entertainment", enters $500
...
│   ├── FAQ → Scroll to FAQ
│   ├── Log In → /login (future — CTA only, no auth backend)
│   └── Get Started Free → /register (future — CTA only)
├── Hero Section (3D animations, video bg, stats)
├── Features Section (6 feature cards)
├── 3D Showcase Section (dark, animated stats, video bg)
├── How It Works (4-step process)
├── Pricing Section (Free/Pro/Business tiers)
├── Review Section (community feedback, video bg)
├── FAQ Section (accordion, 16 items)
├── CTA Section (dark amber, mouse-tracking glow)
└── Footer (links, community hub, 12 platforms, newsletter)
...
|---------|---------|---------|
| react-router-dom | ^7.5.1 | Client-side routing (18+ routes: `/`, `/dashboard/*`, 16 pages) |
...
| `useCurrency` | `hooks/useCurrency.js` | Multi-currency pricing (USD/EUR/GBP/INR with auto-detect) |
...
**Trial & Refund Policy (Website):**
- Paid plans include a 14-day free trial — cancel anytime during the trial at no charge
- Once a payment has been made, no refunds will be issued under any circumstances
- Users may cancel their subscription at any time; access continues until the billing period ends
...
- "Join Our Global Community" section with world map + 12 social platforms
- 12 community platforms: WhatsApp, Telegram, Discord, YouTube, LinkedIn, Instagram, GitHub, X, Reddit, Threads, Facebook, Quora
...
8. WEBSITE PATTERNS — Multi-currency, responsive breakpoints, global-first design
...
6. Build `hooks/useCurrency.js` (USD/EUR/GBP/INR auto-detect + pricing data)
7. Build page sections (in order):
   - `Navbar.jsx` — Glass-morphism, theme toggle, mobile hamburger
   - `HeroSection.jsx` — Particles, wireframe, floating coins, credit card, stats, video bg
   - `FeaturesSection.jsx` — 6 feature cards with 3D hover
   - `Showcase3DSection.jsx` — Dark section, video bg, animated dashboard, holographic scanner
   - `HowItWorksSection.jsx` — 4-step process with connectors
   - `PricingSection.jsx` — 3 tiers ($0 / $9.99 / $29.99), 14-day trial, no refunds
   - `ReviewSection.jsx` — Community feedback, video bg, 3 reviews, submission form
   - `FAQSection.jsx` — 16 items with Radix accordion
   - `CTASection.jsx` — Dark amber theme, mouse-tracking glow, trust badges
   - `Footer.jsx` — Links, newsletter, 12 social platforms, community hub
...
### Success Criteria
- [ ] All 10 sections render correctly (incl. Review)
...
| Supabase Pro + regional compliance | Medium | 📋 Planned | Before production launch |
...
## Update: February 2026 — Dynamic Currency System (Dashboard)

### Overview
The dashboard's currency system is fully dynamic. The user selects their preferred currency in Settings; that choice persists in `localStorage` and automatically propagates to every page, chart, and calculator in the app without any page refresh.

### Architecture

```
Settings.jsx  →  updateSettings({ currency: '$' })  →  saveToStorage(STORAGE_KEYS.SETTINGS, ...)
      ↓
FinanceContext.jsx  →  settings.currency || '$'  →  exposed via useFinance()
      ↓
All pages/components  →  const { currency } = useFinance()  →  formatCurrency(amount, currency)
```
...
| File | Role |
|------|------|
| `context/FinanceContext.jsx` | Source of truth — loads settings from localStorage, exposes `currency` and `dateFormat` via context |
| `utils/helpers.js` | `formatCurrency(amount, currency = '$')` — locale `en-US`, default symbol `$` |
| `utils/localStorage.js` | `STORAGE_KEYS.SETTINGS` — persists user preferences |
| `pages/Settings.jsx` | UI for changing currency and date format; calls `updateSettings()` |

### localStorage Migration (Auto-Applied on Load)

On each app load, `FinanceContext` auto-migrates stale defaults so existing users see USD instead of a leftover INR value:

```js
// FinanceContext.jsx — init useEffect
...
### Dashboard Currency Coverage

| Component | How Currency Is Used |
|-----------|---------------------|
| `Dashboard.jsx` | `formatCurrency(total, currency)` — balance, income, expenses, savings cards |
| `Transactions.jsx` | `formatCurrency(amount, currency)` — totals and per-row amounts |
| `Income.jsx` | `formatCurrency(amount, currency)` |
| `Expenses.jsx` | `formatCurrency(amount, currency)` |
| `Budgets.jsx` | `formatCurrency(amount, currency)` |
| `Goals.jsx` | `formatCurrency(amount, currency)` |
| `Investments.jsx` | `formatCurrency(amount, currency)` |
| `Reports.jsx` | `formatCurrency(amount, currency)` |
| `Bills.jsx` | `formatCurrency(amount, currency)` |
| `Calculators.jsx` | `const { currency } = useFinance()` — EMI, SIP, Compound Interest results |
| `ExpensePieChart` | `currency` prop passed from Dashboard + Reports callers |
| `IncomeExpenseChart` | `currency` prop passed from Reports caller |
| `TrendLineChart` | `currency` prop passed from Reports caller |
| `PortfolioChart` | `currency` prop passed from Investments caller |

### Settings Currency Options (ordered, USD first)

```jsx
// Settings.jsx — currency picker
{ value: '$',  label: '$ US Dollar (USD)' },   // default
...
### Date Format Options (ordered, MM/DD/YYYY first)

```jsx
// Settings.jsx — date format picker
{ value: 'MM/DD/YYYY', label: 'MM/DD/YYYY (US)' },   // default
{ value: 'DD/MM/YYYY', label: 'DD/MM/YYYY (EU)' },
{ value: 'YYYY-MM-DD', label: 'YYYY-MM-DD (ISO)' },
```

**Status:** Dynamic Currency System Complete ✅ — USD default, user-selectable, propagates to all 10 dashboard pages, 4 chart components, and calculators.

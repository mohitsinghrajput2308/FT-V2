...
import {
    ...
    Eye, EyeOff, Copy, Check, Trash2, Info, Clock, Key,
...
const Profile = () => {
    ...
    const [secData, setSecData] = useState({
        ...
        custom_question: '',
        security_answer: currentUser?.security_answer || '',
        show_answer: false,
    });
    ...
    const handleSecuritySubmit = async (e) => {
        ...
        const effectiveQuestion =
            secData.security_question === 'Other (custom question)'
                ? secData.custom_question.trim()
                : secData.security_question;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-1">
                    <div className="text-center space-y-3">
                        ...
                        {/* Short User ID */}
                        <div className="pt-2 border-t border-gray-100 dark:border-dark-400">
                            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1 font-medium uppercase tracking-wide">User ID</p>
                            <div className="flex items-center justify-between gap-2 bg-gray-50 dark:bg-dark-200 rounded-lg px-3 py-2">
                                <span className="text-xs font-mono text-gray-600 dark:text-gray-300 truncate">{shortId}</span>
                                <button
                                    onClick={handleCopyId}
                                    title="Copy full User ID"
                                    className="text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 flex-shrink-0 transition-colors"
                                >
                ...
                <Card>
                    ...
                    <form onSubmit={handleSecuritySubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    type={secData.show_answer ? 'text' : 'password'}
                                    name="security_answer"
                                    value={secData.security_answer}
                                    onChange={handleSecChange}
                                    placeholder="Enter your answer"
                                    className="w-full px-3 py-2 pr-10 text-sm border border-gray-300 dark:border-dark-400 rounded-lg bg-white dark:bg-dark-200 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                                />
                                <button
                                    type="button"
                                    onClick={() => setSecData(p => ({ ...p, show_answer: !p.show_answer }))}
                                    className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                                >
                                    {secData.show_answer ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                </button>
                            </div>
                        </div>
            ...
            <Card>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="sm:col-span-2 lg:col-span-2 bg-gray-50 dark:bg-dark-200 rounded-lg p-3">
                        <p className="text-xs text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wide mb-1">Full User ID</p>
                        <div className="flex items-center gap-2">
                            <span className="text-xs font-mono text-gray-600 dark:text-gray-300 break-all">{currentUser?.id || '—'}</span>
                            <button
                                onClick={handleCopyId}
                                title="Copy User ID"
                                className="text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 flex-shrink-0 transition-colors"
